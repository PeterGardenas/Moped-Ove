...
int irqStartCount = 0;
int resdefer;
boolean mail_sender = true;
boolean mail_receiver = false;
boolean POLLING_FLAG = false;
boolean DwcXferTaskSet = false;
struct usb_xfer_request *req_again;
...
dwc_get_free_channel(void)
{
    ...
    im = disable();
//    printf("infor: wait channel\r\n");
//    wait(chfree_sema);
//    usb_sem_wait(&chfree_sema, 0);
...
dwc_release_channel(uint chan)
{
    ...
    im = disable();
    chfree |= (1 << chan);
//    printf("infor: signal channel\r\n");
//    semsignal(chfree_sema);
//    pi_printf("infor: pre signal channel\r\n");
//    usb_sem_signal(&chfree_sema);
    restore(im);
}
...
{
    int retval;
    //usb_info("Powering on Synopsys DesignWare Hi-Speed USB 2.0 On-The-Go Controller\n");
    retval = bcm2835_SetPower(POWER_USB, TRUE);
...
//             "USB 2.0 On-The-Go Controller\n");
    pi_printf("infor: power off synopsys designWare Hi-Speed\r\n");
...
{
    const unsigned int rx_words = 1024;  /* Size of Rx FIFO in 4-byte words */
    const unsigned int tx_words = 1024;  /* Size of Non-periodic Tx FIFO in 4-byte words */
    const unsigned int ptx_words = 1024; /* Size of Periodic Tx FIFO in 4-byte words */
...
//    usb_debug("original FIFO sizes: rx 0x%08x,  tx 0x%08x, ptx 0x%08x\n", regs->rx_fifo_size, regs->nonperiodic_tx_fifo_size, regs->host_periodic_tx_fifo_size);
//    pi_printf("infor: original FIFO sizes "); pi_print(regs->rx_fifo_size, 0); pi_print( regs->nonperiodic_tx_fifo_size, 0); pi_print(regs->host_periodic_tx_fifo_size, 1);
...
//        pi_printf("infor: host port status changed\r\n");
...
//        pi_printf("infor: simulating request to root hub's default endpoint\r\n");
...
//        pi_printf("infor: posting status change request to root hub\r\n");
...
dwc_channel_start_transaction(uint chan, struct usb_xfer_request *req)
{
    ...
    interrupt_mask.transaction_error = 0;
...
//            	printf("Device: %u, Starting SETUP transaction\r\n", req->dev->address);
...
//            	printf("Device: %u, Starting DATA transactions\r\n", req->dev->address);
...
//            	printf("Device: %u, Starting STATUS transaction\r\n", req->dev->address);
    ...
    if (IS_WORD_ALIGNED(data))
    {
        ...
        chanptr->dma_address = (unsigned int)data;
    }
    else
    {
        ...
        chanptr->dma_address = (unsigned int)aligned_bufs[chan];
...
//    printf("Device: %u, Setting up transactions on channel %u:\r\nmax_packet_size=%u, endpoint_number=%u, endpoint_direction=%s\r\nlow_speed=%u, endpoint_type=%s, device_address=%u\r\n",
//			req->dev->address, chan, characteristics.max_packet_size,
//            characteristics.endpoint_number,  usb_direction_to_string(characteristics.endpoint_direction),
//            characteristics.low_speed, usb_transfer_type_to_string(characteristics.endpoint_type), characteristics.device_address);
//    printf("size=%u, packet_count=%u, packet_id=%u, split_enable=%u, complete_split=%u\r\n",
//			transfer.size, transfer.packet_count,
//			transfer.packet_id, split_control.split_enable, req->complete_split);
 ...
 */
void DeferXferTask(void)
{
//    uint interval_ms;
    uint chan;
    struct usb_xfer_request *req = req_again;
//    if (req->dev->speed == USB_SPEED_HIGH)
//    {
...
//		WaitEvent(EVENT_MASK_DeferXferEvent);
		if (POLLING_FLAG == true) {
			...
			if (req->need_sof) {
				...
				pi_printf("infor: waiting for start-of-frame\r\n");
				im = disable();
				...
				//receive();
				pi_printf("infor: DeferXferTask receive()\r\n");
...
#endif /* START_SPLIT_INTR_TRANSFERS_ON_SOF */
			{
				...
				pi_printf("infor: waiting to start xfer again\r\n");
				//sleep(interval_ms);
				chan = dwc_get_free_channel();
				dwc_channel_start_xfer(chan, req);
			}
		}
//		CleanEvent(EVENT_MASK_DeferXferEvent);
	}
}
...
//	pi_printf("infor: deferring transfer\r\n");
	req_again = req;
    ...
    if (SYSERR == req->deferer_thread_sema)
    {
        ...
        if (SYSERR == req->deferer_thread_sema)
        {
        	...
        	pi_printf("error: can't create semaphore\r\n");
            return USB_STATUS_OUT_OF_MEMORY;
        }
    }
    if (BADTID == req->deferer_thread_tid)
    {
//    	pi_printf("infor: every time?\r\n");
    	//defer_xfer_thread(req);
...
//            usb_dev_error(req->dev, "Can't create thread to service periodic transfer\n");
//            return USB_STATUS_OUT_OF_MEMORY;
//        }
    }
    POLLING_FLAG = true;
...
//    printf("Device %u, %u packets transferred on channel %u\r\n", req->dev->address, packets_transferred, chan);
...
//                      bytes_transferred);
//        printf("Device %u, Calculated %u bytes transferred\r\n", req->dev->address, bytes_transferred);
...
//                              interrupts.val, chanptr->transfer.val);
            	pi_printf("error: transfer complete flag not set\r\n");
                return XFER_FAILED;
            }
...
//            printf("Device %u, Transfer completed on channel %u\r\n",  req->dev->address, chan);
        ...
        else
        {
            ...
            //pi_printf("infor: continuing transfer complete_split = "); mini_uart_sendDec(req->complete_split); pi_printf("\r\n");
    ...
    else
    {
        ...
        else
        {
        	...
        	pi_printf("error: no packets transferred\r\n");
            return XFER_FAILED;
        }
    }
}
...
//			chan, interrupts.val, chanptr->characteristics.val,	chanptr->transfer.val);
...
//                      chan, interrupts.val, chanptr->transfer.packet_count);
		printf("Device %u, Transfer error on channel %u (interrupts pending: 0x%08x, packet_count=%u)\r\n",
				req->dev->address, chan, interrupts.val,
				chanptr->transfer.packet_count);

        if( ETH_OPEN_FLAG == false ){
        	Wdg_SetTriggerCondition(1);
        	bcm2835_Sleep(1000);
        }
...
//                      "restarting transaction\n", chan);
        pi_printf("error: frame overrun on channel\r\n");
    ...
    else if (interrupts.nyet_response_received)
    {
        ...
        pi_printf("error: NYET response received on channel\r\n");
...
//    	printf("Received SOF intr (host_frame_number=0x%08x)\r\n", regs->host_frame_number);
    	if ((regs->host_frame_number & 0x7) != 6)
        {
            ...
            if (sofwait != 0)
            {
                ...
                pi_printf("infor: wake up channel\r\n");
                sofwait &= ~(1 << chan);
            }
...
//        pi_printf("infor: port interrupt detected "); pi_print( hw_status.val, 1 );
...
dwc_setup_interrupts(void)
{
    ...
    /* Enable the interrupt line that goes to the USB controller and register
   	...
   	ISR_INSTALL_ISR2("USB", USB_Isr, BCM2835_IRQ_ID_USB, 9, 0);
   	IRQ_ENABLE1 = USB_IRQ;
...
void DwcXferTask(void) {

	uint chan;
	struct usb_xfer_request *req;

	for (;;) {
//		pi_printf("DwcXferTask\r\n");
		/* Get next transfer request.  */
		WaitEvent(EVENT_MASK_DwcXferEvent);
	    ClearEvent(EVENT_MASK_DwcXferEvent);
...
dwc_start_xfer_scheduler(void)
{
	...
	err = usb_sem_new(&chfree_sema, DWC_NUM_CHANNELS);
//	if (err != SEM_OK) {
//		pi_printf("error: fail to create chfree sema\r\n");
...
hcd_start(void)
{
    ...
    if (status != USB_STATUS_SUCCESS)
    {
    	pi_printf("error: fail to power on USB 2.0 controller\r\n");
        return status;
    }
    dwc_soft_reset();
    dwc_setup_dma_mode();
...
hcd_submit_xfer_request(struct usb_xfer_request *req)
{
	...
	if(activate == false){
		ActivateTask(TASK_ID_DwcXferTask);
		activate = true;
	}
	SetEvent(TASK_ID_DwcXferTask,EVENT_MASK_DwcXferEvent);
    return USB_STATUS_SUCCESS;
}
