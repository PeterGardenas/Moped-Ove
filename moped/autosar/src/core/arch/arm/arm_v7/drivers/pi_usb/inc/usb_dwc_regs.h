...
#ifndef USB_DWC_REGS_H_
#define USB_DWC_REGS_H_
...
{
    STATIC_ASSERT(offset(struct dwc_regs, vendor_id) == 0x40);
    STATIC_ASSERT(offset(struct dwc_regs, host_periodic_tx_fifo_size) == 0x100);
    STATIC_ASSERT(offset(struct dwc_regs, host_configuration) == 0x400);
    STATIC_ASSERT(offset(struct dwc_regs, host_port_ctrlstatus) == 0x440);
    STATIC_ASSERT(offset(struct dwc_regs, reserved_0x800) == 0x800);
    STATIC_ASSERT(offset(struct dwc_regs, power) == 0xe00);
}

extern void ResetHandler(void);
#define restart ResetHandler

#endif /* USB_DWC_REGS_H_ */
