...
#ifndef CRC_32_MODE
#error "CRC_32_MODE is not defined in configuration"
#elif CRC_32_MODE == CRC_32_HARDWARE
...
static const uint32 crc32_tab[] = { 0x00000000,0x77073096,0xEE0E612C,0x990951BA,0x076DC419,0x706AF48F,0xE963A535,0x9E6495A3,
...
uint32 Crc_CalculateCRC32( const uint8* Crc_DataPtr, uint32 Crc_Length, uint32 Crc_StartValue32, boolean Crc_IsFirstCall ) {
    ...
    if (Crc_DataPtr != NULL) {
        ...
        for( uint32 byte = 0; byte < Crc_Length; byte++) {
            crc = ((crc >> 8) & 0x00FFFFFFU) ^ crc32_tab[(crc ^ *Crc_DataPtr) & 0xFFU];
            Crc_DataPtr++;
        }
#endif
