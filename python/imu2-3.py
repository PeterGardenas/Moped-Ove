...
b=bus.read_byte_data(address, 0x75)
print("read byte %#x" % b)
...
Write_Sensor(MPU9150_I2C_SLV0_REG, 0x0);
...
Write_Sensor(MPU9150_I2C_SLV1_ADDR, 0x8C);
# Set where reading at slave 1 starts
Write_Sensor(MPU9150_I2C_SLV1_REG, 0x03);
...
def readgyro():
    while True:
        if False:
            ...
            y = make_word(w[2], w[3])
            z = make_word(w[4], w[5])
            w = bus.read_i2c_block_data(address, 0x43, 6)
            p = make_word(w[0], w[1])
            q = make_word(w[2], w[3])
            r = make_word(w[4], w[5])
        else:
            w = bus.read_i2c_block_data(address, 0x3b, 6+2+6+1+6)
            x = make_word(w[0], w[1])
            y = make_word(w[2], w[3])
            z = make_word(w[4], w[5])
            p = make_word(w[8], w[9])
            ...
            mx = make_word(w[15], w[16])
            my = make_word(w[17], w[18])
            mz = make_word(w[19], w[20])

        print("%d %d %d %d %d %d %d %d %d %s %s" % (p, q, r, x, y, z, mx, my, mz, marker, speed))
