...
def postImage(url, mac):
    """
    Post an image to the server.
    """
    print("Posting image...")
    try:
        response = requests.post(url + "/processimage", params = {"mac" : mac}, files = {"file": ("", open("/dev/shm/optiposimage.jpg", "rb"), "image/jpeg")}).text
    except Exception:
...
def writePositionToCAN(canSocket, canFrameID, posX, posY, orientation, qualityFactor, delay):
    ...
    print("QF = " + str(max(100, int(qualityFactor * 100))))
