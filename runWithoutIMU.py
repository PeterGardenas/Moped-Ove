# !!! README !!!
# ----------------------------------------------------
# This version has the IMU code commented out of it
# and therefore blocks use of hardware functions
...
wm.wminit()
nav1.nav1init()
#EDITif not g.simulate:
#EDIT    nav_imu.imuinit()
if not g.simulate:
    nav_mqtt.mqttinit()
else:
    nav_mqtt.mqttinit()
...
#EDIT    if not g.simulate:
#EDIT     		nav_imu.calibrate_imu()
...
#EDIT    if not g.simulate:
#EDIT        start_new_thread_really(nav_imu.readgyro, ())
    if not g.simulate:
